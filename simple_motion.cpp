	...
	while (1) {
		...
		*/
               ...
               if(new_cmd.Request)
               {
				...
				rt_task_wait_period(NULL);
                                now = rt_timer_read() / 1000000000;
                                t = now - previous_0;
				printf("Task has been running for %ld.%06ld s\n",
					(long)t , (long)(now - previous_0) % 1000000000);
                                //Judge the motion phase
                                if(t <= t1){
                                        axis1_setpoint.Position = 0.5 * new_cmd.Acceleration * t * t;
                                        axis1_setpoint.Velocity = new_cmd.Acceleration * t;       
                                }else if(t <= t2){
                                        axis1_setpoint.Position = 0.5 * new_cmd.Acceleration * (new_cmd.Velocity / new_cmd.Acceleration) * t1 + 
                                                                  new_cmd.Velocity * (t - new_cmd.Velocity / new_cmd.Acceleration);
                                        axis1_setpoint.Velocity = new_cmd.Velocity;
                                }else if(t <= t3){
                                        axis1_setpoint.Position = ((new_cmd.Position - (new_cmd.Velocity * new_cmd.Velocity) / new_cmd.Acceleration) / new_cmd.Velocity + 
                                                                  new_cmd.Velocity/ new_cmd.Acceleration) * new_cmd.Velocity - 0.5 * new_cmd.Deceleration * (t3 - t) * (t3 - t);
                                        axis1_setpoint.Velocity = new_cmd.Velocity - new_cmd.Deceleration * (t - t2);
                                }else {
                                        new_cmd.Done = true;
                                        new_cmd.Response = false;
                                }
                                printf("axis1's position: %lf.\naxis1's velocity: %lf\n\n", axis1_setpoint.Position, axis1_setpoint.Velocity);
                        }
               }
	}
}
